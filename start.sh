...
if screen -list | grep -q '\.hero\s'; then
    echo "Server is already started!  Press screen -r hero to open it"
    exit 1
fi

# Change directory to server directory
cd /home/hero154/minecraftbe/hero
...
Permissions=$(sudo bash /home/hero154/minecraftbe/hero/fixpermissions.sh -a)

# Create backup
if [ -d "worlds" ]; then
    echo "Backing up server (to minecraftbe/hero/backups folder)"
    if [ -n "$(which pigz)" ]; then
        echo "Backing up server (multiple cores) to minecraftbe/hero/backups folder"
        tar -I pigz -pvcf backups/$(date +%Y.%m.%d.%H.%M.%S).tar.gz worlds
    else
        echo "Backing up server (single cored) to minecraftbe/hero/backups folder"
        tar -pzvcf backups/$(date +%Y.%m.%d.%H.%M.%S).tar.gz worlds
    fi
	echo "Syncing backups to S3..."
    rclone sync /home/hero154/minecraftbe/hero/backups tebi:hero111/backups
fi

# Rotate backups -- keep most recent 10
Rotate=$(
    pushd /home/hero154/minecraftbe/hero/backups
...
if [ "$?" != 0 ]; then
    ...
    if [[ "$PinFile" == *"zip" ]] && [[ "$InstalledFile" == "$PinFile" ]]; then
        ...
        if [ ! -z "$DownloadFile" ]; then
            if [ ! -e /home/hero154/minecraftbe/hero/server.properties ]; then
            ...
            Permissions=$(chmod u+x /home/hero154/minecraftbe/hero/bedrock_server >/dev/null)
...
if [ ! -e /home/hero154/minecraftbe/hero/allowlist.json ]; then
    echo "Creating default allowlist.json..."
    echo '[]' >/home/hero154/minecraftbe/hero/allowlist.json
fi
if [ ! -e /home/hero154/minecraftbe/hero/permissions.json ]; then
    echo "Creating default permissions.json..."
    echo '[]' >/home/hero154/minecraftbe/hero/permissions.json
fi
ContentLogging=$(grep "content-log-file-enabled" /home/hero154/minecraftbe/hero/server.properties)
if [ -z "$ContentLogging" ]; then
    echo "" >> /home/hero154/minecraftbe/hero/server.properties
    echo "content-log-file-enabled=true" >> /home/hero154/minecraftbe/hero/server.properties
    echo "# Enables logging content errors to a file" >> /home/hero154/minecraftbe/hero/server.properties
fi

echo "Starting Minecraft server.  To view window type screen -r hero"
...
if [[ "$CPUArch" == *"aarch64"* ]]; then
    ...
    if [ -n "$(which box64)" ]; then
        ...
        BASH_CMD="LD_LIBRARY_PATH=/home/hero154/minecraftbe/hero /home/hero154/minecraftbe/hero/bedrock_server"
    fi
else
    BASH_CMD="LD_LIBRARY_PATH=/home/hero154/minecraftbe/hero /home/hero154/minecraftbe/hero/bedrock_server"
fi
...
screen -L -Logfile logs/hero.$(date +%Y.%m.%d.%H.%M.%S).log -dmS hero /bin/bash -c "${BASH_CMD}"
